...
@OnlyIn(Dist.CLIENT)
public class CustomSheetedDecalTextureGenerator extends DefaultedVertexConsumer {
	...
	public CustomSheetedDecalTextureGenerator(VertexConsumer p_85880_, Matrix4f p_85881_, Matrix3f p_85882_) {
	...
	public void endVertex() {
		...
		float f1 = -vector4f.y();
		double mul = 0.1;
		this.delegate.vertex((double) this.x + (Math.random() - 0.5) * mul, (double) this.y + (Math.random() - 0.5) * mul, (double) this.z + (Math.random() - 0.5) * mul).color(1.0F, 1.0F, 1.0F, 1.0F).uv(f, f1)
				.overlayCoords(this.overlayU, this.overlayV).uv2(this.lightCoords).normal(this.nx, this.ny, this.nz).endVertex();
		this.resetState();
	}
