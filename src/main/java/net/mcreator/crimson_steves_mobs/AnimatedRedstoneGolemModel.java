...
@OnlyIn(Dist.CLIENT)
public class AnimatedRedstoneGolemModel<T extends LivingEntity> extends HierarchicalModel<T> implements ArmedModel, HeadedModel {
	...
	private final ModelPart[] leftFinger = new ModelPart[3];
	protected final ModelPart rightShoulder;
	protected final ModelPart rightWrist;
	private final ModelPart[] rightFinger = new ModelPart[3];
	protected final ModelPart leftLeg;
	protected final ModelPart rightLeg;
	protected final float pi = (float) Math.PI;
	private final List<ModelPart> BlockParts;
	private final List<ModelPart> BodyParts;

	public AnimatedRedstoneGolemModel(ModelPart root) {
		...
		for (int i = 0; i < 3; i++) {
			this.leftFinger[i] = leftWrist.getChild(getSegmentName1(i));
			this.rightFinger[i] = rightWrist.getChild(getSegmentName2(i));
		}
		this.leftLeg = whole.getChild("leftLeg");
		this.rightLeg = whole.getChild("rightLeg");
		this.BlockParts = ImmutableList.of(redstoneBlock);
		this.BodyParts = ImmutableList.of(head.getChild("head_r"), upperBody.getChild("body_r1"), lowerBody.getChild("pelvis_r1"), leftLeg, rightLeg, leftShoulder, leftWrist, rightShoulder, rightWrist);
	}

	public List<ModelPart> getBlockLayerModelParts() {
	...
	private static String getSegmentName1(int p_170706_) {
		return "leftFinger" + (p_170706_ + 1);
	}

	private static String getSegmentName2(int p_170706_) {
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition head_r1 = head.addOrReplaceChild("head_r", CubeListBuilder.create().texOffs(0, 0).addBox(-8.0F, -3.75F, -10.0F, 16.0F, 12.0F, 16.0F), PartPose.offsetAndRotation(0.0F, -2.0F, -8.0F, 1.5708F, 0.0F, 0.0F));
	...
	protected float walk1;
	protected float walk2;
	protected float shiftDownAmount;
	protected float attackReadyAnimationAmount;
	protected float modifiedAttackReadyAnimationAmount;
	...
	protected void setAttackReadyAmount(T e) {
		attackReadyAnimationAmount = e.getPersistentData().getFloat("attackReadyAnimationAmount");
		modifiedAttackReadyAnimationAmount = (attackReadyAnimationAmount + partialTicks) / 10;
		modifiedAttackReadyAnimationAmount = modifiedAttackReadyAnimationAmount * modifiedAttackReadyAnimationAmount;
		modifiedAttackReadyAnimationAmount = Mth.sin(modifiedAttackReadyAnimationAmount * pi);
	}
	...
	public void setupAnim(T e, float f, float f1, float f2, float f3, float f4) {
		...
		if ("lumpy".equals(e.getName().getString()))
			redstoneBlock.offsetScale(new Vector3f(0.75f, 0.75f, 0.75f));
		if ("wreck-it".equals(e.getName().getString())) {
			leftWrist.offsetScale(new Vector3f(1.25f, 0f, 1.25f));
			rightWrist.offsetScale(new Vector3f(1.25f, 0f, 1.25f));
		}
		idle = (Mth.sin(f2 * 0.03f) + 1) * Mth.clamp(1 - f1, 0, 1);
		pitch = f4 * (pi / 180F);
		headYaw = f3 * (pi / 180F);
		walk = (Mth.sin(f) + 1) * f1;
		walk1 = (Mth.cos(f * 0.5f) + 1) * f1;
		walk2 = (-Mth.cos(f * 0.5f) + 1) * f1;
		shiftDownAmount = e.getPersistentData().getFloat("ShiftDownAmount");
		setAttackReadyAmount(e);
		//basic settings
		...
		//
		//AttackAnimation
		if (attackReadyAnimationAmount > 0) {
			if (flag1) {
				this.rightShoulder.xRot += modifiedAttackReadyAnimationAmount;
				this.rightShoulder.yRot += modifiedAttackReadyAnimationAmount;
				this.upperBody.yRot += modifiedAttackReadyAnimationAmount;
				this.lowerBody.yRot += modifiedAttackReadyAnimationAmount * 0.5f;
			} else {
				this.leftShoulder.xRot += modifiedAttackReadyAnimationAmount;
				this.leftShoulder.yRot -= modifiedAttackReadyAnimationAmount;
				this.upperBody.yRot -= modifiedAttackReadyAnimationAmount;
				this.lowerBody.yRot -= modifiedAttackReadyAnimationAmount * 0.5f;
			}
		}
		if (this.attackTime > 0) {
			...
			if (this.getAttackArm(e) == HumanoidArm.RIGHT) {
				...
				this.rightShoulder.zRot += -Mth.sin(a * pi * 2) * 2;
				...
				this.leftShoulder.zRot += Mth.sin(a * pi * 2) * 2;
				//this.rightShoulder.yRot = (headYaw + 1) * b;
				this.leftShoulder.xRot *= 1 - b;
				//this.rightWrist.zRot += b;
			}
		...
		if (shiftDownAmount > 0) {
			this.redstoneBlock.x += (float) (Math.random() - 0.5) * 3 * shiftDownAmount;
			this.redstoneBlock.y += (float) (Math.random() - 0.5) * 3 * shiftDownAmount;
			this.redstoneBlock.z += (Mth.sin(f2 * 1.7f) + 1) * 2 * shiftDownAmount;
			this.lowerBody.xRot *= 1 - shiftDownAmount;
			this.lowerBody.xRot += shiftDownAmount * pi * 5 / 18;
			this.lowerBody.x += (float) (Math.random() - 0.5) * 2 * shiftDownAmount;
			this.lowerBody.z += (float) (Math.random() - 0.5) * 2 * shiftDownAmount;
			this.upperBody.xRot += shiftDownAmount * pi / 18;
			this.upperBody.yRot *= 1 - shiftDownAmount;
			this.upperBody.x += (float) (Math.random() - 0.5) * 2 * shiftDownAmount;
			this.upperBody.z += (float) (Math.random() - 0.5) * 2 * shiftDownAmount;
			this.leftShoulder.xRot *= 1 - shiftDownAmount;
			this.leftShoulder.xRot += -pi * shiftDownAmount * 5 / 18;
			this.leftShoulder.yRot *= 1 - shiftDownAmount;
			this.leftShoulder.yRot += -0.1f * shiftDownAmount;
			this.leftShoulder.x += (float) (Math.random() - 0.5) * 3 * shiftDownAmount;
			this.leftShoulder.z += (float) (Math.random() - 0.5) * 3 * shiftDownAmount;
			this.leftWrist.xRot *= 1 - shiftDownAmount;
			this.leftWrist.xRot += -pi * shiftDownAmount / 9;
			this.leftWrist.yRot *= 1 - shiftDownAmount;
			this.leftWrist.yRot += 0.4f * shiftDownAmount;
			this.rightShoulder.xRot *= 1 - shiftDownAmount;
			this.rightShoulder.xRot += -pi * shiftDownAmount * 5 / 18;
			this.rightShoulder.yRot *= 1 - shiftDownAmount;
			this.rightShoulder.yRot += 0.1f * shiftDownAmount;
			this.rightShoulder.x += (float) (Math.random() - 0.5) * 3 * shiftDownAmount;
			this.rightShoulder.z += (float) (Math.random() - 0.5) * 3 * shiftDownAmount;
			this.rightWrist.xRot *= 1 - shiftDownAmount;
			this.rightWrist.xRot += -pi * shiftDownAmount / 9;
			this.rightWrist.yRot *= 1 - shiftDownAmount;
			this.rightWrist.yRot += -0.4f * shiftDownAmount;
			if (this.attackTime > 0) {
				float a = 1 - this.attackTime;
				a = a * a * a;
				float b = Mth.sin(a * pi) * shiftDownAmount;
		...
		/*
		if (e.isShiftKeyDown()) {
			shiftDownAmount = Mth.lerp(0.04f, shiftDownAmount, 1);
		} else if (shiftDownAmount > 0) {
			shiftDownAmount = Mth.lerp(0.04f, shiftDownAmount, 0);
		}
		if (shiftDownAmount != e.getPersistentData().getFloat("ShiftDownAmount")) {
			e.getPersistentData().putFloat("ShiftDownAmount", shiftDownAmount);
		}
		*/
	}
