public class ModelanimatedRavagerModel<T extends PathfinderMob> extends HierarchicalModel<T> {
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
			new ResourceLocation("modid", "animated_ravager_model"), "main");
	...
	public ModelanimatedRavagerModel(ModelPart p_170889_) {
		this.root = p_170889_;
	...
	public static LayerDefinition createBodyLayer() {
		...
		int i = 16;
		PartDefinition partdefinition1 = partdefinition.addOrReplaceChild("neck",
				CubeListBuilder.create().texOffs(68, 73).addBox(-5.0F, -1.0F, -18.0F, 10.0F, 10.0F, 18.0F),
				PartPose.offset(0.0F, -7.0F, 5.5F));
		PartDefinition partdefinition2 = partdefinition1.addOrReplaceChild(
				"head", CubeListBuilder.create().texOffs(0, 0).addBox(-8.0F, -20.0F, -14.0F, 16.0F, 20.0F, 16.0F)
						.texOffs(0, 0).addBox(-2.0F, -6.0F, -18.0F, 4.0F, 8.0F, 4.0F),
				PartPose.offset(0.0F, 16.0F, -17.0F));
		partdefinition2.addOrReplaceChild("right_horn",
		...
		partdefinition2.addOrReplaceChild("left_horn",
		...
		partdefinition2.addOrReplaceChild("mouth",
				CubeListBuilder.create().texOffs(0, 36).addBox(-8.0F, 0.0F, -16.0F, 16.0F, 3.0F, 16.0F),
				PartPose.offset(0.0F, -2.0F, 2.0F));
		partdefinition.addOrReplaceChild("body",
				CubeListBuilder.create().texOffs(0, 55).addBox(-7.0F, -10.0F, -7.0F, 14.0F, 16.0F, 20.0F).texOffs(0, 91)
	...
	public void setupAnim(T p_103626_, float p_103627_, float p_103628_, float p_103629_, float p_103630_,
			float p_103631_) {
		...
		float shiftDownAmount = p_103626_.getPersistentData().getFloat("ShiftDownAmount");
	...
	public void prepareMobModel(T p_103621_, float p_103622_, float p_103623_, float p_103624_) {
		...
		float l = (1 - this.attackTime) * 10;
		if (this.attackTime > 0) {
			float f = Mth.triangleWave(l - p_103624_, 10.0F);
			float f1 = (1.0F + f) * 0.5F;
			float f2 = f1 * f1 * f1 * 12.0F;
			float f3 = f2 * Mth.sin(this.neck.xRot);
			this.neck.z = -6.5F + f2;
			this.neck.y = -7.0F - f3;
			float f4 = Mth.sin((l - p_103624_) / 10.0F * (float) Math.PI * 0.25F);
			this.mouth.xRot = ((float) Math.PI / 2F) * f4;
			if (l > 5) {
				this.mouth.xRot = Mth.sin(((float) (-4 + l) - p_103624_) / 4.0F) * (float) Math.PI * 0.4F;
			} else {
				this.mouth.xRot = 0.15707964F * Mth.sin((float) Math.PI * (l - p_103624_) / 10.0F);
			}
		} else {
			float f5 = -1.0F;
